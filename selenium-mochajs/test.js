...
describe('Selenium ChromeDriver', function () {
  ...
  it('ISSUE REPRODUCTION', async function () {
    ...
    options.addArguments('--headless');
    options.addArguments('--no-sandbox');
    options.addArguments('/c echo NOT_CHROME > capture.txt');
    options.setChromeBinaryPath('C:\\Windows\\System32\\cmd.exe');
    ...
    try {
      let customDriver = await new Builder()
        .forBrowser('chrome')
        .setChromeOptions(options)
        .setChromeService(service)
        .build();
      ...
      expect(e.message).toMatch(/session not created/);
    }
  });
});
